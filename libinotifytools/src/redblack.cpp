...
*/
struct RB_ENTRY(node) RB_ENTRY(_null) = {&RB_ENTRY(_null), &RB_ENTRY(_null),
					 &RB_ENTRY(_null), BLACK};
...
static void RB_ENTRY(_left_rotate)(struct RB_ENTRY(node) **,
				   struct RB_ENTRY(node) *);
static void RB_ENTRY(_right_rotate)(struct RB_ENTRY(node) **,
				    struct RB_ENTRY(node) *);
static struct RB_ENTRY(node) *
    RB_ENTRY(_successor)(const struct RB_ENTRY(node) *);
static struct RB_ENTRY(node) *
    RB_ENTRY(_predecessor)(const struct RB_ENTRY(node) *);
static struct RB_ENTRY(node) *
    RB_ENTRY(_traverse)(int, const RB_ENTRY(data_t) *, struct RB_ENTRY(tree) *);

/* These functions may not be needed */
#ifndef no_lookup
static struct RB_ENTRY(node) *
    RB_ENTRY(_lookup)(int, const RB_ENTRY(data_t) *, struct RB_ENTRY(tree) *);
#endif
...
#ifndef no_delete
static void RB_ENTRY(_delete)(struct RB_ENTRY(node) **,
			      struct RB_ENTRY(node) *);
static void RB_ENTRY(_delete_fix)(struct RB_ENTRY(node) **,
				  struct RB_ENTRY(node) *);
#endif

#ifndef no_walk
static void RB_ENTRY(_walk)(
    const struct RB_ENTRY(node) *,
...
RB_STATIC struct RB_ENTRY(tree) *
    rbinit(int (*cmp)(const char*, const char*, const void*),
	   const void* config)
#else
...
#endif /* RB_CUSTOMIZE */
{
	...
	if ((retval = (struct RB_ENTRY(tree)*)malloc(
		 sizeof(struct RB_ENTRY(tree)))) == NULL)
		return (NULL);
...
#ifndef no_destroy
RB_STATIC void RB_ENTRY(destroy)(struct RB_ENTRY(tree) * rbinfo) {
	if (rbinfo == NULL)
		return;
...
RB_STATIC const RB_ENTRY(data_t) *
    RB_ENTRY(search)(const RB_ENTRY(data_t) * key,
...
#ifndef no_find
RB_STATIC const RB_ENTRY(data_t) *
    RB_ENTRY(find)(const RB_ENTRY(data_t) * key,
		   struct RB_ENTRY(tree) * rbinfo) {
	struct RB_ENTRY(node) * x;
...
#ifndef no_delete
RB_STATIC const RB_ENTRY(data_t) *
    RB_ENTRY(delete)(const RB_ENTRY(data_t) * key,
		     struct RB_ENTRY(tree) * rbinfo) {
	struct RB_ENTRY(node) * x;
...
#ifndef no_walk
RB_STATIC void RB_ENTRY(walk)(
    const struct RB_ENTRY(tree) * rbinfo,
    void (*action)(const RB_ENTRY(data_t) *, const VISIT, const int, void*),
    void* arg) {
	if (rbinfo == NULL)
		return;
...
#ifndef no_readlist
RB_STATIC RBLIST* RB_ENTRY(openlist)(const struct RB_ENTRY(tree) * rbinfo) {
	if (rbinfo == NULL)
		return (NULL);
...
RB_STATIC const RB_ENTRY(data_t) * RB_ENTRY(readlist)(RBLIST* rblistp) {
	if (rblistp == NULL)
		return (NULL);
...
RB_STATIC void RB_ENTRY(closelist)(RBLIST* rblistp) {
	if (rblistp == NULL)
		return;
...
static struct RB_ENTRY(node) *
	...
	while (x != RBNULL && found == 0) {
		 ...
		 */
#ifndef RB_CUSTOMIZE
		cmp = RB_CMP((const char*)key, RB_GET((const char*)x, key),
			     rbinfo->rb_config);
#else
...
#ifndef RB_CUSTOMIZE
		cmp = RB_CMP(RB_GET((const char*)z, key),
			     RB_GET((const char*)y, key), rbinfo->rb_config);
#else
	...
	while (x != rbinfo->rb_root && (x->up->colour == RED)) {
		...
		if (x->up == x->up->up->left) {
			...
			if (y->colour == RED) {
				...
				RB_ENTRY(_right_rotate)
				(&rbinfo->rb_root, x->up->up);
			}
		} else {
 ...
 */
static void RB_ENTRY(_destroy)(struct RB_ENTRY(node) * x) {
	if (x != RBNULL) {
		if (x->left != RBNULL)
...
static void RB_ENTRY(_left_rotate)(struct RB_ENTRY(node) * *rootp,
				   struct RB_ENTRY(node) * x) {
...
static void RB_ENTRY(_right_rotate)(struct RB_ENTRY(node) * *rootp,
				    struct RB_ENTRY(node) * y) {
	struct RB_ENTRY(node) * x;
 ...
 */
static void RB_ENTRY(_delete)(struct RB_ENTRY(node) * *rootp,
...
static void RB_ENTRY(_delete_fix)(struct RB_ENTRY(node) * *rootp,
				  struct RB_ENTRY(node) * x) {
...
#ifndef no_readlist
static RBLIST* RB_ENTRY(_openlist)(const struct RB_ENTRY(node) * rootp) {
	RBLIST* rblistp;
...
static const RB_ENTRY(data_t) * RB_ENTRY(_readlist)(RBLIST* rblistp) {
...
static struct RB_ENTRY(node) * RB_ENTRY(_alloc)() {
	...
	if (rbfreep == NULL) {
		...
		rbfreep = (struct RB_ENTRY(node)*)sbrk(
		    sizeof(struct RB_ENTRY(node)) * RB_ENTRY(NODE)
							ALLOC_CHUNK_SIZE);
		...
		for (i = 0, x = rbfreep;
		     i < RB_ENTRY(NODE) ALLOC_CHUNK_SIZE - 1; i++, x++) {
			x->up = (x + 1);
		}
		x->up = NULL;
	}
...
/* free (dealloc) an RB_ENTRY(node) structure - add it onto the front of the
 *list
